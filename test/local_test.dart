...
void main() {
  ...
  setUp(() async {
    ...
    localDataServiceLocations = LocalDataServiceLocationsImpl(sharedPreferences: sharedPreferences);
    localDataServiceEpisodes = LocalDataServiceEpisodesImpl(sharedPreferences: sharedPreferences);
    localDataServicePersons = LocalDataServicePersonsImpl(sharedPreferences: sharedPreferences);
  });

  group('LocalDataServiceLocationsImpl', () {
    test('should add locations to cache and set last page', () async {
      final List<LocationDto> locations = [
        LocationDto(id: 1, name: 'Earth', type: 'Planet', dimension: 'C-137', residents: [], url: 'https://rickandmortyapi.com/api/location/1', created: '2017-11-10T12:42:04.162Z'),
        LocationDto(id: 2, name: 'Abadango', type: 'Cluster', dimension: 'unknown', residents: [], url: 'https://rickandmortyapi.com/api/location/2', created: '2017-11-10T13:06:38.182Z'),
      ];

      await localDataServiceLocations.addDtosToCache(locations, 1);

      final List<String> cachedLocations = sharedPreferences.getStringList(CACHED_LOCATION_LIST) ?? [];
      final int lastPage = sharedPreferences.getInt(CACHED_LOCATION_LAST_PAGE) ?? 0;
    ...
    test('should return last locations from cache', () async {
      ...
      await sharedPreferences.setStringList(CACHED_LOCATION_LIST, jsonLocations);
    ...
    test('should throw CacheException when there are no locations in cache', () async {
      await sharedPreferences.remove(CACHED_LOCATION_LIST);

      expect(() => localDataServiceLocations.getLastDtosFromCache(), throwsA(isA<CacheException>()));
    });
  ...
  group('LocalDataServiceEpisodesImpl', () {
    test('should add episodes to cache and set last page', () async {
      final List<EpisodeDto> episodes = [
        EpisodeDto(id: 1, name: 'Pilot', airDate: 'December 2, 2013', episode: 'S01E01', characters: [], url: 'https://rickandmortyapi.com/api/episode/1',),
        EpisodeDto(id: 2, name: 'Lawnmower Dog', airDate: 'December 9, 2013', episode: 'S01E02', characters: [], url: 'https://rickandmortyapi.com/api/episode/2', ),
      ];

      await localDataServiceEpisodes.addDtosToCache(episodes, 1);

      final List<String> cachedEpisodes = sharedPreferences.getStringList(CACHED_EPISODE_LIST) ?? [];
      final int lastPage = sharedPreferences.getInt(CACHED_EPISODE_LAST_PAGE) ?? 0;
    ...
    test('should throw CacheException when there are no episodes in cache', () async {
      await sharedPreferences.remove(CACHED_EPISODE_LIST);

      expect(() => localDataServiceEpisodes.getLastDtosFromCache(), throwsA(isA<CacheException>()));
    });
  ...
  group('LocalDataServicePersonsImpl', () {
    test('should add persons to cache and set last page', () async {
      const locationMap = <String,dynamic> {"name": "Alien Spa",
        "url": "https://rickandmortyapi.com/api/location/64"};
      final List<PersonDto> persons = [
        PersonDto(id: 1, name: 'Rick Sanchez', status: 'Alive', species: 'Human', type: '', gender: 'Male', origin:locationMap , location: locationMap, image: 'https://rickandmortyapi.com/api/character/avatar/1.jpeg', episode: [], url: 'https://rickandmortyapi.com/api/character/1', created:  DateTime.parse('2017-11-04T18:48:46.250Z')),
        PersonDto(id: 2, name: 'Morty Smith', status: 'Alive', species: 'Human', type: '', gender: 'Male', origin: locationMap, location: locationMap, image: 'https://rickandmortyapi.com/api/character/avatar/2.jpeg', episode: [], url: 'https://rickandmortyapi.com/api/character/2', created: DateTime.parse('2017-11-04T18:48:46.250Z')),
      ];

      await localDataServicePersons.addDtosToCache(persons, 1);

      final List<String> cachedPersons = sharedPreferences.getStringList(CACHED_PERSONS_LIST) ?? [];
      final int lastPage = sharedPreferences.getInt(CACHED_PERSONS_LAST_PAGE) ?? 0;
    ...
    test('should throw CacheException when there are no persons in cache', () async {
      await sharedPreferences.remove(CACHED_PERSONS_LIST);

      expect(() => localDataServicePersons.getLastDtosFromCache(), throwsA(isA<CacheException>()));
    });
